        ...
        <div className="grid gap-x-10 gap-y-12 xl:grid-cols-[1.45fr_1fr_1fr_1fr]">
          ...
          <section aria-label={content.trust}>
            ...
            <div className="grid grid-cols-2 gap-2" aria-label={content.paymentSecurity}>
              <div aria-label="Visa" className="flex min-h-12 items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 text-xs font-bold text-white"><CreditCard className="h-4 w-4 text-[var(--brand-gold)]" aria-hidden="true" /><span>Visa</span></div>
              <div aria-label="Mastercard" className="flex min-h-12 items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 text-xs font-bold text-white"><CreditCard className="h-4 w-4 text-[var(--brand-gold)]" aria-hidden="true" /><span>Mastercard</span></div>
              <div aria-label={language === 'ar' ? 'الدفع عند الاستلام' : language === 'fr' ? 'Paiement à la remise' : 'Pay at pickup'} className="flex min-h-12 items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 text-xs font-bold text-white"><Banknote className="h-4 w-4 text-[var(--brand-gold)]" aria-hidden="true" /><span>{language === 'ar' ? 'الدفع عند الاستلام' : language === 'fr' ? 'Paiement à la remise' : 'Pay at pickup'}</span></div>
              <div aria-label={language === 'ar' ? 'عقد رقمي' : language === 'fr' ? 'Contrat numérique' : 'Digital contract'} className="flex min-h-12 items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 text-xs font-bold text-white"><FileSignature className="h-4 w-4 text-[var(--brand-gold)]" aria-hidden="true" /><span>{language === 'ar' ? 'عقد رقمي' : language === 'fr' ? 'Contrat numérique' : 'Digital contract'}</span></div>
            </div>
